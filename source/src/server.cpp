...
void enddemorecord()
{
    ...
    { // finish recording, assemble game data,
        ...
        formatstring(demorecord->info)("%s: %s, %s, %.2f%s", asctime(), modestr(gamemode), sg->smapname, len > 1024*1024 ? len/(1024*1024.f) : len/1024.0f, len > 1024*1024 ? "MB" : "kB");
...
void setupdemorecord()
{
    ...
    defformatstring(desc)("%s, %s, %s %s", modestr(gamemode, false), behindpath(sg->smapname), asctime(), sg->servdesc_current);
    if(strlen(desc) > DHDR_DESCCHARS)
        formatstring(desc)("%s, %s, %s %s", modestr(gamemode, true), behindpath(sg->smapname), asctime(), sg->servdesc_current);
...
void loggamestatus(const char *reason)
{
    ...
    mlog(ACLOG_INFO, "cn name             %s%s score frag death %sping role    host", m_teammode ? "team " : "", m_flags ? "flag " : "", m_teammode ? "tk " : "");
    loopv(clients)
    {
        client &c = *clients[i];
    ...
    if(m_teammode)
    {
        loopi(2) mlog(ACLOG_INFO, "Team %4s:%3d players,%5d frags%c%5d flags", team_string(i), pnum[i], fragscore[i], m_flags ? ',' : '\0', flagscore[i]);
    }
    mlog(ACLOG_INFO, "");
}
