...
!define AC_PROFILEPATH "AssaultCube_v${AC_MAJORVERSIONINT}.${AC_MINORVERSIONINT}"
...
!define lookfor "bulletairsound 0"
Var found
...
Section "AssaultCube ${AC_FULLVERSIONINT}" AC
    ...
    ; fix mouse glitch on Windows 10 Fall Creators Update, fixed in April 2018 Update
    ${WinVerGetBuild} $0
    ${If} ${AtLeastWin10}
        ${If} $0 >= 16299
        ${AndIf} $0 < 17134
            Rename $INSTDIR\bin_win32\SDL.dll $INSTDIR\bin_win32\SDL-1.2.14.dll
            Rename $INSTDIR\bin_win32\SDL-1.2.13.dll $INSTDIR\bin_win32\SDL.dll
        ${EndIf}
    ${EndIf}

    ; workaround for sound bug on Windows (sounds stop to play)
    SetShellVarContext current
    StrCpy $1 $DOCUMENTS
    SetShellVarContext all
    StrCpy $found 0
    ${LineFind} "$1\${AC_PROFILEPATH}\config\autoexec.cfg" "/NUL" "1:-1" "GrepFunc"
    ${if} $found = 0
        FileOpen $2 "$1\${AC_PROFILEPATH}\config\autoexec.cfg" a
        FileSeek $2 0 END
        FileWrite $2 "$\r$\n"
        FileWrite $2 "bulletairsound 0"
        FileWrite $2 "$\r$\n"
        FileClose $2
    ${endIf}
